...
class DomainCrawler(scrapy.Spider):
  ...
  def __init__(self, user_settings, custom_crawl_rules = [], name: str | None = None, **kwargs: Any):
    super(DomainCrawler, self).__init__(name, **kwargs)
  ...
  def parse(self, response):
    ...
    if ("text/html" in converted_headers["Content-Type"]):
      ...
      # Rule for specific page
      for rule in self.custom_crawl_rules:
        if rule[0] in response.url:
          url_custom_rule = rule[1]
          print(rule[0])
          break
        
      # Rule for main page
      if len(url_custom_rule) == 0:
        for rule in self.custom_crawl_rules:
          if rule[0] == "#":
            protocol1 = 'https://'
            protocol2 = 'http://'   
            ...
            for domain in self.allowed_domains:
              is_main_page_1 = response.url.find(protocol1 + domain) >= 0 and len(response.url) == len(protocol1 + domain)
              is_main_page_2 = response.url.find(protocol1 + domain + '/') >= 0 and len(response.url) == len(protocol1 + domain + '/')
              is_main_page_3 = response.url.find(protocol2 + domain) >= 0 and len(response.url) == len(protocol2 + domain)
              is_main_page_4 = response.url.find(protocol2 + domain + '/') >= 0 and len(response.url) == len(protocol2 + domain + '/')
              
              if is_main_page_1 or is_main_page_2 or is_main_page_3 or is_main_page_4:
                url_custom_rule = rule[1]
                print(rule[0])
                break
            break
      ...
      # Custom Content
      for customRule in url_custom_rule:
